...
async fn run_valence_iter_orchestrator_impl(
    ...
    let run = ValenceIterRun::get_used(&run_id, &valence, valence::use_!("get ValenceIterRun in src/iter/orchestrator.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
        .await
        .map_err(|e| anyhow!("{}", e))?
    ...
    ValenceIterRun::merge_used(
        ...
        &valence,
        valence::use_!("merge ValenceIterRun in src/iter/orchestrator.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."),
    )
    .await
    .map_err(|e| anyhow!("{}", e))?;
    ...
    loop {
        let run_row = ValenceIterRun::get_used(&run_id, &valence, valence::use_!("get ValenceIterRun in src/iter/orchestrator.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
            .await
            .map_err(|e| anyhow!("{}", e))?
        ...
        .map_err(|e| anyhow!("{}", e))?;
        ValenceIterBatch::upsert_used(&batch_id, batch, &valence, valence::use_!("upsert ValenceIterBatch in src/iter/orchestrator.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
            .await
            .map_err(|e| anyhow!("{}", e))?;
        ...
        for rid in &page {
            ...
            ValenceIterBatch::merge_used(
                ...
                &valence,
                valence::use_!("merge ValenceIterBatch in src/iter/orchestrator.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."),
            )
            .await
            .map_err(|e| anyhow!("{}", e))?;
        }
        ...
        let scanned_so_far = *ValenceIterRun::get_used(&run_id, &valence, valence::use_!("get ValenceIterRun in src/iter/orchestrator.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
            .await
            .map_err(|e| anyhow!("{}", e))?
            .context("run missing")?
            .scanned_rows();
    ...
    loop {
        let r = ValenceIterRun::get_used(&run_id, &valence, valence::use_!("get ValenceIterRun in src/iter/orchestrator.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
            .await
            .map_err(|e| anyhow!("{}", e))?
            .context("run missing")?;
